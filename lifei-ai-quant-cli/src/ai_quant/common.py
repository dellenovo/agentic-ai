...
_ALIASES = {
    ...
    # 现金流量表
    "销售收现": ["销售商品、提供劳务收到的现金"],
    "经营现金流": ["经营活动产生的现金流量净额", "经营活动产生/(使用)的现金流量净额", "经营活动产生/（使用）的现金流量净额"],
    "投资现金流": ["投资活动产生的现金流量净额", "投资活动产生/(使用)的现金流量净额", "投资活动产生/（使用）的现金流量净额", "投资活动使用的现金流量净额"],
    "筹资现金流": ["筹资活动产生的现金流量净额", "筹资活动(使用)/产生的现金流量净额", "筹资活动（使用）/产生的现金流量净额", "筹资活动产生/（使用）的现金流量净额"],
    "资本开支": ["购建固定资产", "购建固定资产、无形资产和其他长期资产支付的现金"],
    "期末现金": ["期末现金及现金等价物余额", "现金及现金等价物的年末余额", "年末现金及现金等价物余额"],
    # 美的等公司财务费用可能称为"财务收入"（净收入）
    "财务费用": ["财务费用", "财务收入", "财务(费用)/收入"],
    "合同负债": ["合同负债"],
    "研发费用": ["研发费用"],
    "应收款项融资": ["应收款项融资"],
}
...
_STMT_OF = {
    ...
    "所有者权益合计": "balance_sheet", "应收款项融资": "balance_sheet",
...
def subj(fin: dict, canonical: str):
    ...
    匹配优先级：按别名候选列表顺序，每个候选在表中找**最短**包含它的科目名。
    避免「负债合计」误匹配到更长的「流动负债合计」。
    """
