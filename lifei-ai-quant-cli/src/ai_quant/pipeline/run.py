...
硬约束：L1 最先、L5 最后；中间 L2 出图与 L3 研判并行。L3 研判是人工产出的
analysis/findings.json（非脚本），编排无法自动跑它——进 L5 前做闸门检查：
findings.json 缺失或不完整就明确提示并停下，不静默出半成品报告。

流程：
    L1 解析(PDF → financials.json)
      → L2 出图(financials + findings → figures/*.png)         ┐ 二者无依赖
      → [闸门] 检查 L3 研判产物 analysis/findings.json 是否就位  ┘
      → L5 报告(financials + findings + figures → report.html)
"""
...
def _gate_check_findings(stock_code: str) -> dict:
    """L3 闸门：按股票代码定位研判产物，必须就位且结构完整，否则停下。"""
    ...
    if not fp.exists():
        ...
        raise PipelineError(
            ...
            "  L3 风险研判由分析者（Claude Code）人工产出，按公司存为 analysis/findings_<代码>.json，不是脚本自动生成。\n"
...
def run_pipeline(pdf_path: str, skip_figures: bool = False, stock_code: str = "") -> dict:
    ...
    _log("L3", f"{fp.name} 就位：风险 {len(finds['risk_findings'])} 条、"
    ...
    print("▶ L5 报告")
    out = build_report()
    _log("L5", f"{out.name}（{out.stat().st_size/1024:.0f} KB，图表内嵌）")
