...
<section>
  ...
  {% endfor %}
  <p style="font-size:12px;color:var(--gray);margin-top:10px;">注：金额单位按年报原文；同比红=增长、绿=下降。</p>
</section>
...
<section>
  ...
  {% endfor %}
  {% if cross_checks_round2 %}
  <p style="font-size:13.5px;color:#444;margin-top:14px;">第二轮：用间接法桥（净利润→经营现金流）深挖背离。</p>
...
<section>
  ...
  <p>会计恒等式自检（资产总计 = 负债合计 + 所有者权益合计）：
     期末/期初各两条，
     {% if checks.balance_identity.ok %}<span class="check-ok">✓ 全部通过</span>{% else %}<span class="up">✗ 存在不平</span>{% endif %}。
  </p>
</section>
...
def _kpi(fin):
    ...
    cogs = val(fin, "营业成本")
    cogs_abs = abs(cogs) if cogs else 0
    ...
    return {
        ...
        "ocf": ocf, "gm": (rev - cogs_abs) / rev * 100 if rev else 0, "lev": liab / ta * 100,
    }
