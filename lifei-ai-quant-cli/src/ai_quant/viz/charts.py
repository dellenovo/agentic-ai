...
中文字体：matplotlib 默认无中文字体会乱码（缺字变方框）。
...
def chart_revenue_profit(fin, manifest):
    ...
    # 营业成本可能为负值（如美的年报用括号表示减项），取绝对值
    cogs_c_abs = abs(cogs_c) if cogs_c else 0
    cogs_p_abs = abs(cogs_p) if cogs_p else 0
    gm_c = (rev_c_raw - cogs_c_abs) / rev_c_raw * 100 if rev_c_raw else 0
    gm_p = (rev_p_raw - cogs_p_abs) / rev_p_raw * 100 if rev_p_raw else 0
    nm_c = ni_c / rev_c * 100
    nm_p = ni_p / rev_p * 100
    ...
    _save(fig, "revenue_profit", "营收与净利润",
          "净利润增速与毛利率、净利率变化，观察盈利质量趋势。", manifest)
